...
public class GetPhotoFromPhotoAlbum {
    ...
    public static String getRealPathFromUri(final Context context, final Uri uri) {
        ...
        if (sdkVersion >= 19) {
            ...
            path = getRealPathFromUriBelowAPI19(context, uri);
        }
        return path;
    }
     ...
     * @return 如果Uri对应的图片存在, 那么返回该图片的绝对路径, 否则返回null
     */
    private static String getRealPathFromUriBelowAPI19(final Context context, final Uri uri) {
     ...
     */
    @SuppressLint("NewApi")
    private static String getRealPathFromUriAboveApi19(final Context context, final Uri uri) {
//        Context c = createPackageContext("com.example.a86182.icamera", Context.CONTEXT_INCLUDE_CODE | Context.CONTEXT_IGNORE_SECURITY);
        ...
        if (DocumentsContract.isDocumentUri(context, uri)) {
            ...
            if (isMediaDocument(uri)) {
                // 使用':'分割
                final String idArray = documentId.split(":")[1];

                final String selection = MediaStore.Images.Media._ID + "=?";
                final String[] selectionArgs = {idArray};
                filePath = getDataColumn(context, MediaStore.Images.Media.EXTERNAL_CONTENT_URI, selection, selectionArgs);
            }
            else if (isDownloadsDocument(uri)) {
                final Uri uriTemp = Uri.parse("content://downloads/public_downloads");
                final long docID =  Long.parseLong(documentId);
                final Uri contentUri = ContentUris.withAppendedId(uriTemp,docID);
